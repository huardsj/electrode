...
async function searchRoutesDir(srcDir, pluginOpts) {
    ...
    console.log('searchRoutesDir ---> ', { options, dir: routesDir, routes });
...
async function setupRoutesFromFile(srcDir, server, pluginOpts) {
    ...
    server.addHook('onRequest', async (request, reply) => {
        const rte = routes[request.path];
        if (rte && rte.initialize) {
            const x = await rte.initialize(request, reply);
            if (x) return x;
        }
        return;
    });
        ...
        const handler = async (request, reply) => {
            try {
                ...
                if (data instanceof Error) {
                    ...
                    return reply.type("text/html; charset=UTF-8").code(200).send(data);
                } else if (HttpStatus.redirect[status]) {
                    return reply.code(status).send(data.path);
                } else if (HttpStatus.displayHtml[status]) {
                    return reply.code(status).send(data.html !== undefined ? data.html : data);
                ...
                return errorResponse({ routeName: path, request, h: reply, err });
            }
        };
...
async function setupSubAppFastifyRoutes (server, pluginOpts , done) {
    const srcDir =
        pluginOpts.srcDir ||
    ...
    await xaa.wrap(() => setupRoutesFromFile(srcDir, server, pluginOpts));
    return done && done();
}

module.exports = {
    setupSubAppFastifyRoutes
};
